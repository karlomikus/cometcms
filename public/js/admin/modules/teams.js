...
new Vue({
    ...
    methods: {
        removeFromMembers: function removeFromMembers(index) {
...
var SquadViewModel = function SquadViewModel(squadData) {
    var self = this;
    ...
    if (squadData.roster.length > 0) {
        $.each(squadData.roster, function (key, val) {
            if (!jQuery.isEmptyObject(val.pivot)) self.members.push(new SquadMemberViewModel(self, val));
        });
    } else {
...
var SquadMemberViewModel = function SquadMemberViewModel(parent, memberData) {
    ...
    self.addToMembers = function () {
        ...
        var exists = $.grep(parent.members(), function (e) {
            return e.user_id() === self.user_id();
        });
        if (exists.length === 0) parent.members.push(self);
    };
